...
const ipLimiter = new RateLimiter(Number(process.env.NP_IP_PER_MINUTE ?? 20), Number(process.env.NP_IP_PER_DAY ?? 300));
...
const app = createApp({ fetcher, limiter, ipLimiter, allowedHosts, fetchesPerCheck: Number(process.env.NP_FETCHES_PER_CHECK ?? 2), breaker, stats, crowd });
...
setInterval(() => { limiter.sweep(); ipLimiter.sweep(); }, 60 * 60 * 1000).unref();
